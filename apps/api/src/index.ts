...
import {
  mergeIssabelPbxApiPatch,
  parseIssabelPbxApiJson,
...
function safeParseJsonObject(raw: string | null): Record<string, unknown> {
  if (!raw?.trim()) return {};
  try {
...
function parseExtensionCreateBody(raw: unknown):
  | { ok: false; err: z.ZodError }
  | {
      ok: true;
  ...
  if (!parsed.success) return { ok: false, err: parsed.error };
  const obj = raw as Record<string, unknown>;
  const meta: Record<string, unknown> = { ...(parsed.data.metadata ?? {}) };
  for (const [k, v] of Object.entries(obj)) {
    if (!EXT_CREATE_CORE.has(k)) meta[k] = v;
  }
...
function parseExtensionPatchBody(
  raw: unknown,
  existingMetaStr: string | null,
): | { ok: false; err: z.ZodError } | { ok: true; core: z.infer<typeof extensionPatchCoreZ>; metadataStr: string | null } {
  const obj = raw as Record<string, unknown>;
  const patchCore = extensionPatchCoreZ.safeParse(obj);
  if (!patchCore.success) return { ok: false, err: patchCore.error };
  const next: Record<string, unknown> = { ...safeParseJsonObject(existingMetaStr) };
  for (const [k, v] of Object.entries(obj)) {
    if (EXT_PATCH_CORE.has(k)) continue;
    next[k] = v;
  }
...
async function syncPortalExtensionToIssabel(
  orgRow: typeof schema.organizations.$inferSelect,
  extRow: typeof schema.extensions.$inferSelect,
  mode: 'create' | 'update',
): Promise<typeof schema.extensions.$inferSelect> {
  const cfg = parseIssabelPbxApiJson(orgRow.issabelPbxApi ?? undefined);
  if (!cfg) return extRow;
  ...
  const nextSource = sync.ok ? ('synced' as const) : extRow.source;
  await db
...
async function getSessionUser(c: any) {
  ...
  try {
    ...
    if (!u) return null;
    return { ...u, tokenRole: role as (typeof schema.users.$inferSelect)['role'] };
  } catch {
    return null;
  }
}
...
async function membershipForOrg(userId: number, organizationId: number) {
  const [m] = await db
    .select()
...
async function canReadOrg(u: SessionUser, organizationId: number) {
  if (u.role === 'platform_admin') return true;
  return !!(await membershipForOrg(u.id, organizationId));
}

async function canWriteOrg(u: SessionUser, organizationId: number) {
  if (u.role === 'platform_admin') return true;
...
async function canAdminOrg(u: SessionUser, organizationId: number) {
  if (u.role === 'platform_admin') return true;
  const m = await membershipForOrg(u.id, organizationId);
...
app.get('/public/branding-by-host', async (c) => {
  ...
  if (!org) return c.json({ organizationId: null, appearance: {} });
...
app.patch('/me', async (c) => {
  ...
  if (b.displayName !== undefined) set.displayName = b.displayName;
...
app.get('/organizations', async (c) => {
  ...
  if (u.role !== 'platform_admin') {
    ...
    if (allowed.length === 0) return c.json({ items: [], total: 0, page, pageSize });
    conditions.push(inArray(schema.organizations.id, allowed));
  }
  if (search) {
    conditions.push(
      or(like(schema.organizations.name, `%${search}%`), like(schema.organizations.tradeName, `%${search}%`))!,
    );
  }
  if (orgKind) {
  ...
  if (whereClause) itemsQ = itemsQ.where(whereClause) as typeof itemsQ;
  const items = await itemsQ;
  let countQ = db.select({ count: sql<number>`count(*)` }).from(schema.organizations);
  if (whereClause) countQ = countQ.where(whereClause) as typeof countQ;
  const [{ count }] = await countQ;

  const ids = items.map((o) => o.id);
  const spaceByOrg = new Map<number, number>();
  const extByOrg = new Map<number, number>();
  if (ids.length > 0) {
    ...
    for (const row of spaceAgg) spaceByOrg.set(row.organizationId, row.n);
    ...
    for (const row of extAgg) extByOrg.set(row.organizationId, row.n);
  }
  ...
  const listItems = items.map((o) => {
    ...
    if (o.appearance) {
      try {
        const ap = JSON.parse(o.appearance) as Record<string, unknown>;
        if (typeof ap.logoUrl === 'string' && ap.logoUrl.trim()) logoUrl = ap.logoUrl.trim();
      } catch {
        /* ignore */
      }
    }
...
app.get('/organizations/:id', async (c) => {
  ...
  if (u.role !== 'platform_admin') {
    const [m] = await db
      ...
      .from(schema.organizationMembers)
      .where(
        and(eq(schema.organizationMembers.userId, u.id), eq(schema.organizationMembers.organizationId, id)),
      );
...
app.patch('/organizations/:id/quotas', async (c) => {
  const u = await getSessionUser(c);
  if (!u || u.role !== 'platform_admin') return c.json({ error: 'forbidden' }, 403);
  const id = Number(c.req.param('id'));
  ...
  if (b.cdrMysql !== undefined && b.cdrMysql !== null && String(b.cdrMysql).trim()) {
    ...
    if (!cfg) return c.json({ error: 'invalid_cdr_mysql_json' }, 400);
  }
  const cdrNormalized =
    b.cdrMysql === undefined ? undefined : b.cdrMysql === null || b.cdrMysql === '' ? null : String(b.cdrMysql).trim() || null;
  let issabelPbxApiNext: string | null | undefined;
...
app.patch('/organizations/:id/appearance', async (c) => {
  ...
  if (!u) return c.json({ error: 'unauthorized' }, 401);
  const id = Number(c.req.param('id'));
  if (u.role !== 'platform_admin' && u.role !== 'org_admin') return c.json({ error: 'forbidden' }, 403);
  if (u.role === 'org_admin') {
    const [m] = await db
      .select()
      .from(schema.organizationMembers)
    ...
    if (!m || m.role !== 'org_admin') return c.json({ error: 'forbidden' }, 403);
  }
...
app.patch('/organizations/:id/custom-domain', async (c) => {
  const u = await getSessionUser(c);
  if (!u || (u.role !== 'platform_admin' && u.role !== 'org_admin')) return c.json({ error: 'forbidden' }, 403);
  const id = Number(c.req.param('id'));
...
app.post('/organizations/:id/custom-domain/verify', async (c) => {
  ...
  const id = Number(c.req.param('id'));
  const [org] = await db.select().from(schema.organizations).where(eq(schema.organizations.id, id));
  if (!org?.domainVerificationToken) return c.json({ error: 'no_token' }, 400);
...
app.get('/organizations/:id/spaces', async (c) => {
  ...
  if (!u) return c.json({ error: 'unauthorized' }, 401);
  const id = Number(c.req.param('id'));
  if (!(await canReadOrg(u, id))) return c.json({ error: 'forbidden' }, 403);
  const q = listQuery.safeParse(c.req.query());
  if (!q.success) return c.json({ error: q.error.flatten() }, 400);
  const { page, pageSize, q: search } = q.data;
  const offset = (page - 1) * pageSize;
  const cond = [eq(schema.spaces.organizationId, id)] as const;
  const where = search
  ...
  const items = await db
    ...
    .limit(pageSize)
    .offset(offset);
  const [{ count }] = await db.select({ count: sql<number>`count(*)` }).from(schema.spaces).where(where);
...
app.post('/organizations/:orgId/spaces', async (c) => {
  ...
  if (!(await canAdminOrg(u, orgId))) return c.json({ error: 'forbidden' }, 403);
  const parsed = spaceCreate.safeParse(await c.req.json());
  if (!parsed.success) return c.json({ error: parsed.error.flatten() }, 400);
  const b = parsed.data;
  const [res] = await db
    .insert(schema.spaces)
    .values({ organizationId: orgId, name: b.name, status: b.status ?? 'active' }) as unknown as [{ insertId: number }];
...
app.patch('/organizations/:orgId/spaces/:spaceId', async (c) => {
  ...
  if (!sp || sp.organizationId !== orgId) return c.json({ error: 'not_found' }, 404);
  const b = parsed.data;
  await db
...
app.get('/metrics/billing-summary', async (c) => {
  ...
  const spaces = await db.select({ count: sql<number>`count(*)` }).from(schema.spaces);
  ...
  try {
    ...
    if (typeof v.pricePerClientUsd === 'number') price = v.pricePerClientUsd;
  } catch {
    /* ignore */
  }
...
app.get('/metrics/telephony-overview', async (c) => {
  ...
  if (u.role !== 'platform_admin') {
    const allowed =
      u.role === 'org_admin' || u.role === 'org_operator' || u.role === 'org_viewer'
        ? (
            await db
        ...
        : [];
    if (orgId && !allowed.includes(orgId)) return c.json({ error: 'forbidden' }, 403);
...
app.get('/metrics/calls-online', async (c) => {
  ...
  if (u.role !== 'platform_admin') {
    ...
    if (filterOrgId && !allowed.has(filterOrgId)) return c.json({ error: 'forbidden' }, 403);
  }
  if (filterOrgId) orgs = orgs.filter((o) => o.id === filterOrgId);
...
app.get('/metrics/cdr/history', async (c) => {
  ...
  const oid = oidOpt ?? 0;
  if (oid && !(await canReadOrg(u, oid))) return c.json({ error: 'forbidden' }, 403);
  ...
  if (!cfg) return c.json({ error: 'cdr_not_configured', items: [], total: 0, page, pageSize });
...
app.get('/reports/cdr-export.csv', async (c) => {
  ...
  const [org] = oid ? await db.select().from(schema.organizations).where(eq(schema.organizations.id, oid)) : [undefined];
  const cfg = org?.cdrMysql?.trim() ? parseCdrConfig(org.cdrMysql) : globalCdrConfigFromEnv();
  if (!cfg) return c.text('cdr_not_configured', 400);
  let accountcode: string | null = null;
  if (org?.cdrMysql?.trim()) {
    try {
      const j = JSON.parse(org.cdrMysql) as Record<string, unknown>;
  ...
  try {
    ...
    const body = items
      .map((r) =>
        [r.calldate, r.src, r.dst, r.duration, r.billsec, r.disposition, r.uniqueid, r.dcontext ?? '', r.accountcode ?? '']
...
app.get('/users', async (c) => {
  ...
  if (u.role === 'platform_admin') {
    const rows = await db
      ...
      .from(schema.users);
    return c.json({ items: rows.map((r) => ({ ...r, orgRole: null as string | null })) });
  }
  const memberships = await db
    .select()
    .from(schema.organizationMembers)
  ...
  if (orgIds.length === 0) return c.json({ items: [] });
...
async function canResetUserPassword(u: SessionUser, targetUserId: number) {
  if (u.role === 'platform_admin') return true;
  if (u.role !== 'org_admin') return false;
  const mine = await db
    .select()
    .from(schema.organizationMembers)
    .where(eq(schema.organizationMembers.userId, u.id));
...
app.post('/users/:id/reset-password', async (c) => {
  ...
  if (!parsed.success) return c.json({ error: parsed.error.flatten() }, 400);
  if (!(await canResetUserPassword(u, id))) return c.json({ error: 'forbidden' }, 403);
...
app.get('/users/export.csv', async (c) => {
  const u = await getSessionUser(c);
  if (!u || (u.role !== 'platform_admin' && u.role !== 'org_admin')) return c.json({ error: 'forbidden' }, 403);
  let rows: { id: number; email: string; displayName: string; role: string; orgRole: string | null }[] = [];
  if (u.role === 'platform_admin') {
    const r = await db
...
app.get('/security/logs', async (c) => {
  ...
  const conds = [];
  if (ip) conds.push(like(schema.securityBlockLog.ip, `%${ip}%`));
  if (blockType === 'manual' || blockType === 'auto') conds.push(eq(schema.securityBlockLog.blockType, blockType));
...
app.get('/diagnostics/summary', async (c) => {
  const u = await getSessionUser(c);
  if (!u || u.role !== 'platform_admin') return c.json({ error: 'forbidden' }, 403);
  const [{ c: orgCount }] = await db.select({ c: sql<number>`count(*)` }).from(schema.organizations);
  const [{ c: userCount }] = await db.select({ c: sql<number>`count(*)` }).from(schema.users);
  const [{ c: spaceCount }] = await db.select({ c: sql<number>`count(*)` }).from(schema.spaces);
  const [{ c: whCount }] = await db.select({ c: sql<number>`count(*)` }).from(schema.webhookEndpoints);
  const [{ c: ruleCount }] = await db.select({ c: sql<number>`count(*)` }).from(schema.callReactionRules);
  return c.json({
    apiVersion: '0.1.0',
    uptimeHint: 'dev',
...
app.post('/extensions', async (c) => {
  ...
  if (!parsed.ok) return c.json({ error: parsed.err.flatten() }, 400);
  const b = parsed;
  ...
  if (dup) return c.json({ error: 'duplicate_number' }, 409);
  const [orgCheck] = await db.select().from(schema.organizations).where(eq(schema.organizations.id, b.organizationId));
  if (!orgCheck) return c.json({ error: 'not_found' }, 404);
  if (orgCheck.extensionsLimit != null) {
    const [{ c: extCount }] = await db.select({ c: sql<number>`count(*)` }).from(schema.extensions).where(eq(schema.extensions.organizationId, b.organizationId));
...
app.patch('/extensions/:id', async (c) => {
  ...
  if (orgRow) next = await syncPortalExtensionToIssabel(orgRow, next, 'update');
  return c.json(next);
});
...
app.patch('/webhooks/endpoints/:id', async (c) => {
  ...
  return c.json({
    ...next,
    eventTypes: JSON.parse(next.eventTypes || '[]') as string[],
  });
});
...
app.post('/internal/telephony/events', async (c) => {
  ...
  const rules = await db
    ...
    .where(
      and(
        eq(schema.callReactionRules.organizationId, body.organizationId),
        eq(schema.callReactionRules.eventType, body.eventType),
        eq(schema.callReactionRules.enabled, true),
      ),
    );
  const results: unknown[] = [];
  for (const rule of rules) {
    if (rule.featureKey && rule.featureKey !== body.featureKey) continue;
...
app.get('/extension-groups', async (c) => {
  ...
  return c.json({ items: rows.map((r) => ({ ...r, extensionIds: JSON.parse(r.extensionIds || '[]') as number[] })) });
});
...
function parseWaCfg(raw: string) {
...
app.get('/whatsapp/status', async (c) => {
  ...
  const activeCount = rows.filter(r => r.status === 'active' && r.enabled).length;
  const defaultGw = rows.find(r => { const c = parseWaCfg(r.config); return c.isDefault === 'true'; }) ?? rows[0];
  const cfg = defaultGw ? parseWaCfg(defaultGw.config) : {};
  const [{ c: extCount }] = await db.select({ c: sql<number>`count(*)` }).from(schema.extensions).where(eq(schema.extensions.organizationId, orgId));
  return c.json({
    gateway: { status: defaultGw?.status ?? 'inactive', phone: cfg.phone ?? null, connected: activeCount > 0 },
    logsCount: 0,
    extensionsCount: Number(extCount),
    activeCount,
  });
});

app.get('/whatsapp/gateways', async (c) => {
  const u = await getSessionUser(c);
  if (!u) return c.json({ error: 'unauthorized' }, 401);
  const orgId = Number(c.req.query('organizationId'));
  if (!orgId) return c.json({ error: 'organizationId required' }, 400);
  ...
  const rows = await db.select().from(schema.integrations)
    .where(and(eq(schema.integrations.organizationId, orgId), eq(schema.integrations.type, 'whatsapp')))
    .orderBy(asc(schema.integrations.id));
  const items = rows.map(r => {
    const cfg = parseWaCfg(r.config);
    return { id: r.id, name: cfg.name ?? '', url: cfg.url ?? '', phone: cfg.phone ?? '', messageTemplate: cfg.messageTemplate ?? '', isDefault: cfg.isDefault === 'true', status: r.status, enabled: r.enabled, createdAt: r.createdAt };
  });
  return c.json({ items });
});
...
app.post('/whatsapp/gateways', async (c) => {
  const u = await getSessionUser(c);
  if (!u) return c.json({ error: 'unauthorized' }, 401);
  const parsed = waGatewaySchema.safeParse(await c.req.json());
  if (!parsed.success) return c.json({ error: parsed.error.flatten() }, 400);
  const b = parsed.data;
  if (!(await canWriteOrg(u, b.organizationId))) return c.json({ error: 'forbidden' }, 403);
  if (b.isDefault) {
    const existing = await db.select().from(schema.integrations).where(and(eq(schema.integrations.organizationId, b.organizationId), eq(schema.integrations.type, 'whatsapp')));
    for (const r of existing) {
      const cfg = parseWaCfg(r.config); cfg.isDefault = 'false';
      await db.update(schema.integrations).set({ config: JSON.stringify(cfg) }).where(eq(schema.integrations.id, r.id));
    }
  }
  const configJson = JSON.stringify({ name: b.name, url: b.url, token: b.token, phone: b.phone, messageTemplate: b.messageTemplate, isDefault: String(b.isDefault) });
  const [res] = await db.insert(schema.integrations).values({ organizationId: b.organizationId, type: 'whatsapp', config: configJson, status: 'active', enabled: true }) as unknown as [{ insertId: number }];
  return c.json({ id: res.insertId, ok: true }, 201);
});

const waGatewayUpdateSchema = z.object({
  name: z.string().min(1).max(128).optional(),
  url: z.string().url().optional(),
...
app.patch('/whatsapp/gateways/:id', async (c) => {
  const u = await getSessionUser(c);
  if (!u) return c.json({ error: 'unauthorized' }, 401);
  const id = Number(c.req.param('id'));
  const [row] = await db.select().from(schema.integrations).where(eq(schema.integrations.id, id));
  if (!row || row.type !== 'whatsapp') return c.json({ error: 'not_found' }, 404);
  if (!(await canWriteOrg(u, row.organizationId))) return c.json({ error: 'forbidden' }, 403);
  const parsed = waGatewayUpdateSchema.safeParse(await c.req.json());
  if (!parsed.success) return c.json({ error: parsed.error.flatten() }, 400);
  const b = parsed.data;
  const cfg = parseWaCfg(row.config);
  if (b.name !== undefined) cfg.name = b.name;
  if (b.url !== undefined) cfg.url = b.url;
  if (b.token !== undefined) cfg.token = b.token;
  if (b.phone !== undefined) cfg.phone = b.phone;
  if (b.messageTemplate !== undefined) cfg.messageTemplate = b.messageTemplate;
  if (b.isDefault !== undefined) {
    if (b.isDefault) {
      const siblings = await db.select().from(schema.integrations).where(and(eq(schema.integrations.organizationId, row.organizationId), eq(schema.integrations.type, 'whatsapp')));
      for (const s of siblings) { if (s.id !== id) { const sc = parseWaCfg(s.config); sc.isDefault = 'false'; await db.update(schema.integrations).set({ config: JSON.stringify(sc) }).where(eq(schema.integrations.id, s.id)); } }
    }
    cfg.isDefault = String(b.isDefault);
  }
  await db.update(schema.integrations).set({ config: JSON.stringify(cfg) }).where(eq(schema.integrations.id, id));
  return c.json({ ok: true });
});

app.delete('/whatsapp/gateways/:id', async (c) => {
  const u = await getSessionUser(c);
  if (!u) return c.json({ error: 'unauthorized' }, 401);
  const id = Number(c.req.param('id'));
  const [row] = await db.select().from(schema.integrations).where(eq(schema.integrations.id, id));
...
app.post('/ai-agents', async (c) => {
  ...
  const [res] = await db
    ...
    .values({ organizationId: b.organizationId, name: b.name, model: b.model, prompt: b.prompt }) as unknown as [{ insertId: number }];
  ...
  await db
    ...
    .set({
      ...
      ...(b.headersTemplate !== undefined
        ? {
            headersTemplate:
              b.headersTemplate === null || b.headersTemplate === undefined
                ? null
                : JSON.stringify(b.headersTemplate as object),
          }
        : {}),
...
app.post('/organizations', async (c) => {
  ...
  if (!parsed.success) return c.json({ error: parsed.error.flatten() }, 400);
  const b = parsed.data;
  let issabelPbxApiInsert: string | null = null;
...
app.patch('/organizations/:id', async (c) => {
  ...
  if (!org) return c.json({ error: 'not_found' }, 404);
  const b = parsed.data;
  let issabelPbxApiMerged: string | null | undefined;
  if (b.issabelPbxApi !== undefined) {
    if (b.issabelPbxApi === null || b.issabelPbxApi.trim() === '') {
...
app.post('/organizations/:orgId/members', async (c) => {
  ...
  if (!usr) return c.json({ error: 'user_not_found' }, 404);
  await db
...
app.post('/users', async (c) => {
  ...
  const [res] = await db
    .insert(schema.users)
    .values({ email: b.email, displayName: b.displayName, passwordHash: hash, role: b.role }) as unknown as [{ insertId: number }];
  const [row] = await db
    .select({ id: schema.users.id, email: schema.users.email, displayName: schema.users.displayName, role: schema.users.role })
    .from(schema.users)
...
app.post('/audio-files', async (c) => {
  ...
  if (!orgId || !name) return c.json({ error: 'organizationId and name are required' }, 400);
  if (!(await canWriteOrg(u, orgId))) return c.json({ error: 'forbidden' }, 403);

  let storedFilename: string | null = null;

  if (file && file instanceof File) {
    if (file.size > MAX_AUDIO_SIZE) return c.json({ error: 'file_too_large', maxBytes: MAX_AUDIO_SIZE }, 413);
    if (!ALLOWED_AUDIO_TYPES.has(file.type)) return c.json({ error: 'invalid_file_type' }, 415);
...
app.get('/audio-files/:id/stream', async (c) => {
  ...
  if (!row) return c.json({ error: 'not_found' }, 404);
  if (!(await canReadOrg(u, row.organizationId))) return c.json({ error: 'forbidden' }, 403);
  if (!row.filename) return c.json({ error: 'no_file' }, 404);
...
app.delete('/audio-files/:id', async (c) => {
  ...
  if (row.filename) {
    await unlink(join(AUDIO_DIR, row.filename)).catch(() => { /* file may already be gone */ });
  }
  return c.json({ ok: true });
});
...
app.get('/metrics/queue-log', async (c) => {
  ...
  const [org] = await db.select().from(schema.organizations).where(eq(schema.organizations.id, orgId));
  const cfg = org?.cdrMysql?.trim() ? parseCdrConfig(org.cdrMysql) : globalCdrConfigFromEnv();
  if (!cfg) return c.json({ error: 'cdr_not_configured', items: [] });
  try {
    ...
    try {
      const [rows] = await conn.query(
        `SELECT time, callid, queuename, agent, event, data1, data2, data3, data4, data5 FROM queue_log WHERE FROM_UNIXTIME(time) BETWEEN ? AND ? ORDER BY time DESC LIMIT ?`,
        [from, to, pageSize],
      );
      return c.json({ items: rows });
    } finally {
