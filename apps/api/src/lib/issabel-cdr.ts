...
export type CdrMysqlConfig = {
  host: string;
  port?: number;
  user: string;
  password: string;
  database: string;
  /** Issabel/Asterisk table name, default `cdr` */
  table?: string;
};

function tableName(cfg: CdrMysqlConfig): string {
  const t = cfg.table?.trim() || 'cdr';
  if (!/^[a-zA-Z0-9_]+$/.test(t)) throw new Error('invalid_cdr_table');
  return t;
}

export function parseCdrConfig(raw: string | null | undefined): CdrMysqlConfig | null {
...
export function globalCdrConfigFromEnv(): CdrMysqlConfig | null {
  const json = process.env.ISSABEL_CDR_MYSQL_JSON?.trim();
  if (json) return parseCdrConfig(json);
...
export type TelephonyOverviewCdr = {
  ...
  source: 'cdr';
  /** Buckets (e.g. 24 hours) — call volume per bucket, not true AMI concurrency */
  simultaneousBuckets: number[];
  bucketMinutes: number;
  onlineCallsEstimate: number;
};

function guessDirection(src: string, dst: string): 'inbound' | 'outbound' {
  const extLike = (s: string) => /^[0-9*#+]{1,8}$/.test(s.replace(/\s/g, ''));
  if (extLike(dst) && !extLike(src)) return 'inbound';
  if (extLike(src) && !extLike(dst)) return 'outbound';
...
export async function fetchCdrHistory(
  cfg: CdrMysqlConfig,
  opts: {
    accountcode?: string | null;
    fromIso: string;
...
): Promise<{ items: CdrHistoryRow[]; total: number }> {
  ...
  try {
    ...
    const [rows] = await conn.query<RowDataPacket[]>(
      `SELECT calldate, src, dst, duration, billsec, disposition, uniqueid, dcontext, accountcode
       FROM ${t}
       WHERE calldate >= ? AND calldate <= ? ${acClause} ${extra}
...
export async function fetchTelephonyFromCdr(
  organizationId: number,
  cfg: CdrMysqlConfig,
  opts?: { accountcode?: string | null },
): Promise<TelephonyOverviewCdr> {
  const t = tableName(cfg);
  const conn = await mysql.createConnection({
    host: cfg.host,
    port: cfg.port ?? 3306,
  ...
  try {
    const ac = opts?.accountcode?.trim();
    const acClause = ac ? ` AND accountcode = ? ` : '';
    const params24: (string | number)[] = [];
    if (ac) params24.push(ac);
    ...
    const agg = aggRows[0] as { total: number; answered: number; inboundish: number; sumBill: number } | undefined;
    ...
    const hourlyParams: (string | number)[] = [];
    if (ac) hourlyParams.push(ac);
    const [hourRows] = await conn.query<RowDataPacket[]>(
      `SELECT HOUR(calldate) AS h, COUNT(*) AS n
       FROM ${t}
       WHERE calldate >= (NOW() - INTERVAL 1 DAY) ${acClause}
    ...
    for (const r of hourRows as { h: number; n: number }[]) {
    ...
    const bucketParams: (string | number)[] = [];
    if (ac) bucketParams.push(ac);
    const [bucketRows] = await conn.query<RowDataPacket[]>(
      `SELECT ${bucketSql} AS b, COUNT(*) AS n
       FROM ${t}
       WHERE calldate >= (NOW() - INTERVAL 1 DAY) ${acClause}
       GROUP BY b`,
      bucketParams,
    ...
    for (const r of bucketRows as { b: number; n: number }[]) {
      const idx = Number(r.b);
      if (idx >= 0 && idx < buckets) simultaneousBuckets[idx] = Number(r.n);
    }

    const recentParams: (string | number)[] = [];
    if (ac) recentParams.push(ac);
    const [recentRows] = await conn.query<RowDataPacket[]>(
      `SELECT calldate, src, dst, duration, billsec, disposition, uniqueid, channel, dcontext
       FROM ${t}
    ...
    const recentCalls = (recentRows as unknown as CdrRow[]).map((r) => {
    ...
    const onlineParams: (string | number)[] = [];
    if (ac) onlineParams.push(ac);
    const [onlineRows] = await conn.query<RowDataPacket[]>(
      `SELECT COUNT(*) AS n FROM ${t}
       WHERE calldate >= (NOW() - INTERVAL 10 MINUTE)
       AND disposition = 'ANSWERED'
       AND billsec = 0
       AND duration > 0 ${acClause}`,
      onlineParams,
    );
    const onlineAgg = onlineRows[0] as { n: number } | undefined;
