import { fetchTelephonyFromCdr, globalCdrConfigFromEnv, parseCdrConfig, } from './issabel-cdr.js';
function telephonyDemoSnapshot(organizationId) {
    ...
    const recentCalls = [
        { id: `${organizationId}-1`, direction: 'inbound', from: '+55 11 9****-1201', to: '201', durationSec: 184 + (byte(1) % 40), disposition: 'answered' },
        { id: `${organizationId}-2`, direction: 'outbound', from: '105', to: '+55 21 9****-8832', durationSec: 62 + (byte(2) % 20), disposition: 'answered' },
        { id: `${organizationId}-3`, direction: 'inbound', from: '+351 2** *** 441', to: 'IVR', durationSec: byte(5) % 4, disposition: 'abandoned' },
        { id: `${organizationId}-4`, direction: 'inbound', from: '+55 11 4***-0092', to: 'Fila Vendas', durationSec: 241, disposition: 'answered' },
        { id: `${organizationId}-5`, direction: 'outbound', from: '302', to: '+55 47 9****-2210', durationSec: 315, disposition: 'answered' },
        { id: `${organizationId}-6`, direction: 'inbound', from: '+55 85 3***-7781', to: '201', durationSec: 128, disposition: 'voicemail' },
    ];
    return {
        organizationId,
        calls24h,
...
function accountcodeFromOrg(org) {
...
export async function resolveTelephonyOverview(org, organizationId) {
    const cfg = org ? cdrMysqlFromOrg(org) : globalCdrConfigFromEnv();
    if (!cfg)
        return telephonyDemoSnapshot(organizationId);
    try {
