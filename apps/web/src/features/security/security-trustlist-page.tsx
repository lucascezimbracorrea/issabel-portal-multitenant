...
type Row = { id: number; ip: string; port: number | null; protocol: string; releasedAt: string };

export function SecurityTrustlistPage() {
  ...
  const list = useQuery({
    queryKey: qk.securityTrustlist(),
    queryFn: () => apiFetch<{ items: Row[] }>('/security/trustlist'),
  });

  const add = useMutation({
    mutationFn: () =>
      apiFetch<Row>('/security/trustlist', {
        method: 'POST',
        body: JSON.stringify({ ip, port: port ? Number(port) : null, protocol: 'udp' }),
      }),
    onSuccess: () => {
      toast.success(t('security.saved'));
      qc.invalidateQueries({ queryKey: qk.securityTrustlist() });
  ...
  const del = useMutation({
    mutationFn: (id: number) => apiFetch<{ ok: boolean }>(`/security/trustlist/${id}`, { method: 'DELETE' }),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: qk.securityTrustlist() });
      qc.invalidateQueries({ queryKey: qk.securityLogs({}) });
    },
  ...
  return (
    <div className="space-y-4">
      <div>
        <h1 className="text-2xl font-semibold tracking-tight">{t('security.trustlistTitle')}</h1>
        <p className="text-sm text-muted-foreground">{t('security.trustlistSubtitle')}</p>
      </div>
      <Card>
        <CardHeader>
          <CardTitle className="text-base">{t('security.trustlistAdd')}</CardTitle>
        </CardHeader>
        <CardContent className="flex flex-wrap items-end gap-3">
          <label className="text-xs font-medium text-muted-foreground">
            IP
            <Input className="mt-1 w-48" value={ip} onChange={(e) => setIp(e.target.value)} />
          </label>
          <label className="text-xs font-medium text-muted-foreground">
            {t('security.port')}
            ...
            <table className="w-full border-collapse text-sm">
              <thead>
                <tr className="border-b border-border bg-muted/40 text-left text-xs font-semibold uppercase text-muted-foreground">
                  ...
                  <th className="p-2">{t('security.releasedAt')}</th>
                  <th className="p-2" />
                </tr>
              </thead>
              <tbody>
                {(list.data?.items ?? []).map((r) => (
                  <tr key={r.id} className="border-b border-border/70">
                    ...
                    <td className="p-2 text-xs">{r.releasedAt}</td>
                    <td className="p-2">
