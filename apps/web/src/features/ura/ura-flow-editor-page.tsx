...
import { UraFlowCanvas } from './ura-flow-canvas';
import type { UraGraph, UraNodeType } from './ura-node-types';
import { PALETTE_NODES, NODE_TYPE_META, createNode, buildInitialGraph } from './ura-node-types';
...
export function UraFlowEditorPage() {
  ...
  useEffect(() => {
    if (ura) {
      try {
        const parsed: UraGraph = JSON.parse(ura.graphJson ?? '{"nodes":[],"edges":[]}');
        if (parsed.nodes.length === 0) {
          let dtmf: DtmfActionRow[] = [];
          try { dtmf = JSON.parse(ura.dtmfActionsJson ?? '[]'); } catch { dtmf = []; }
          setGraph(buildInitialGraph(dtmf));
        } else {
          setGraph(parsed);
        }
      } catch {
    ...
    <div className="flex h-screen flex-col overflow-hidden">
      ...
      <div className="flex shrink-0 items-center justify-between border-b border-border bg-background/95 px-4 py-2 backdrop-blur-sm">
        <div className="flex items-center gap-3">
          <button
            type="button"
            onClick={() => void navigate({ to: '/pbx/features/uras/$uraId', params: { uraId: String(uraId) } })}
          ...
          <span className="font-semibold">
            {isPending ? '…' : (ura?.name ?? t('ura.editFallback'))}
          </span>
          {hasUnsaved && (
